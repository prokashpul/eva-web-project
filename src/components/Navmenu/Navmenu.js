...
const Navmenu = () => {
  ...
  return (
    <nav className="bg-slate-400  w-full  md:h-20 md:flex justify-between items-center md:px-20">
      <div className="flex md:static w-full items-center justify-between py-4">
        <div className="flex items-center">
          <FireIcon className="w-12 mr-2 text-lime-300"></FireIcon>{" "}
          <span className="font-mono font-bold text-5xl">EvaWab</span>
        </div>
        <div className="menu-bar" onClick={() => setMenu(!menuOpen)}>
          {menuOpen ? (
            <MenuAlt2Icon className="w-12 mr-2 md:hidden"></MenuAlt2Icon>
          ) : (
            <XIcon className="w-12 mr-2 md:hidden"></XIcon>
          )}
        </div>
      </div>
      <ul
